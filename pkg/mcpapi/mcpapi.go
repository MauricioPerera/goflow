		...
		},
		{
			...
			"inputSchema": map[string]any{
				...
				"properties": map[string]any{
					...
					"onPauseFlow":             map[string]any{"type": "string", "description": "name of ANOTHER saved flow to run whenever this one PAUSES (e.g. the \"approval\" piece) — its trigger payload is {flowName, runId, resumeToken, pausedStepName}, enough to build a one-click approval link: POST /public/runs/{runId}/resume with header X-Resume-Secret: {resumeToken}, no GOFLOW_API_TOKEN required. Omit to disable (the default). Never chains more than one hop, same as onFailureFlow."},
...
func (h *Handler) handleToolsCall(w http.ResponseWriter, req rawRequest) {
	...
	state, validationErrs, runErr := flowstore.RunWithHistory(&def.Flow, h.BuildRegistry, h.CredStore, h.HistoryStore, h.FlowStore, def.Name, params.Arguments, false, def.OnPauseFlow)
